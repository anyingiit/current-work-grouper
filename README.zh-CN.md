...
- 新开的未分组网页自动加入目标分组，默认名称为 `Current Work`（中文界面下为「当前工作」），可以自定义。
- 优先使用已打开的同名分组；找不到时自动创建一个蓝色分组。
- 点击工具栏图标即可修改分组名称、暂停或恢复，或切换界面语言。
- 不联网、不注入网页脚本、不保存浏览记录。

界面默认为英文，可在弹窗的语言菜单中切换为简体中文。`chrome://extensions` 中显示的扩展名称跟随 Chrome 自身的显示语言。
...
4. 在工具栏的拼图菜单中固定「Current Work · Auto-Group」（Chrome 显示语言为中文时显示为「当前工作 · 自动归组」）。

更新时，用新版本文件覆盖同一个文件夹，再到 `chrome://extensions` 点击此扩展的刷新按钮。保持同一个文件夹可以保留设置。

## 使用

1. 点击工具栏图标（可先在右上角切换为简体中文），输入**目标分组名称**，点击**保存设置**。
...
在弹窗右上角的菜单中选择 **English** 或 **简体中文** 即可切换语言，立即生效。如果从未保存过分组名称，默认名称会跟随语言（`Current Work` 或「当前工作」）。从 1.2.0 之前的版本升级且未保存过名称时，会保留「当前工作」作为已保存的名称。
...
| `storage` | 保存分组名称、开关、界面语言，以及短期待处理的标签 ID |
