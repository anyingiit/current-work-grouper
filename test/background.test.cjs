...
const code=readFileSync(require('node:path').join(__dirname,'../extension/background.js'),'utf8');
function fixture() {
 ...
 const chrome={storage:{local:storage(),session:storage(),onChanged:event('storageChange')},action:{onClicked:event('click'),async setBadgeText(o){calls.push(['badge',o.text])},async setBadgeBackgroundColor(){},async setTitle(){}},runtime:{onInstalled:event('install'),onStartup:event('startup')},windows:{async get(){return {type:'normal'}},async update(id,o){calls.push(['focus',id])}},tabs:{onCreated:event('create'),onUpdated:event('update'),onRemoved:event('remove'),async get(id){if(!tabs.has(id))throw Error();return {...tabs.get(id)}},async move(id,o){tabs.get(id).windowId=o.windowId;calls.push(['move',id,o.windowId])},async update(id,o){Object.assign(tabs.get(id),o)},async group(o){const id=o.groupId??(groups.size+1);if(!groups.has(id))groups.set(id,{id,windowId:tabs.get(o.tabIds[0]).windowId});for(const tid of o.tabIds)tabs.get(tid).groupId=id;calls.push(['group',...o.tabIds]);return id}},tabGroups:{async query(){return [...groups.values()]},async update(id,o){Object.assign(groups.get(id),o)}}};
 const ctx=vm.createContext({chrome,Date,console});vm.runInContext(code,ctx);
 const drain=()=>vm.runInContext('queue',ctx);
 function create(id,extra={}){const t={id,windowId:1,groupId:-1,url:'https://example.com',active:true,...extra};tabs.set(id,t);events.create({...t});}
 return {tabs,groups,events,calls,create,drain,chrome,ctx};
}
(async()=>{
 let count=0;
 async function test(name,fn){await fn();count++;console.log('PASS',name)}
 await test('burst creates one named group',async()=>{const f=fixture();f.create(1);f.create(2);await f.drain();assert.equal(f.groups.size,1);assert.equal(f.groups.get(1).title,'当前工作');assert.equal(f.tabs.get(2).groupId,1)});
 await test('existing group in another window is reused',async()=>{const f=fixture();f.groups.set(8,{id:8,title:'当前工作',windowId:2});f.create(1);await f.drain();assert.equal(f.tabs.get(1).groupId,8);assert.equal(f.tabs.get(1).windowId,2);assert.ok(f.calls.some(x=>x[0]==='focus'&&x[1]===2))});
 await test('same-window group preferred over duplicates',async()=>{const f=fixture();f.groups.set(3,{id:3,title:'当前工作',windowId:2});f.groups.set(8,{id:8,title:'当前工作',windowId:1});f.create(1);await f.drain();assert.equal(f.tabs.get(1).groupId,8)});
 ...
 await test('delayed URL survives service-worker restart',async()=>{const f=fixture();f.create(1,{url:''});await f.drain();assert.equal(f.groups.size,0);const ctx2=vm.createContext({chrome:f.chrome,Date,console});vm.runInContext(code,ctx2);f.tabs.get(1).url='https://example.com';f.events.update(1,{url:'https://example.com'});await vm.runInContext('queue',ctx2);assert.equal(f.tabs.get(1).groupId,1)});
 ...
 await test('custom name creates target and old group stays unchanged',async()=>{const f=fixture();f.create(1);await f.drain();await f.chrome.storage.local.set({groupName:'  项目 A * [设计]  '});f.create(2);await f.drain();assert.equal(f.groups.get(1).title,'当前工作');assert.equal(f.groups.get(2).title,'项目 A * [设计]');assert.equal(f.tabs.get(1).groupId,1)});
 ...
 await test('invalid stored name falls back to default',async()=>{const f=fixture();await f.chrome.storage.local.set({groupName:'  '});f.create(1);await f.drain();assert.equal(f.groups.get(1).title,'当前工作')});
